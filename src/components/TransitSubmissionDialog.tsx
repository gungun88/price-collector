...
import { CheckCircle2, ClipboardList, Copy, Mail, MessageCircle, Plus, Send, X } from "lucide-react";
    ...
    <>
      <div className={className}>
        <button
          ...
          className={`inline-flex items-center justify-center bg-[#dde4e5] font-semibold text-[#2d3435] transition hover:bg-[#cfd8d9] ${buttonSizeClassName} ${buttonClassName}`}
        >
          <Plus className="h-4 w-4" />
          {submitLabel ?? (compactLabels ? (
            <>
        ...
        {showMerchant ? (
          <button
            ...
            className={`inline-flex items-center justify-center bg-[#2d3435] font-semibold text-[#f8f8f8] transition hover:bg-[#1f2526] ${buttonSizeClassName} ${buttonClassName}`}
          >
            {compactLabels ? (
              <>
...
function TransitSubmissionModal({
  ...
  return (
    <div
      className="priceai-square-ui fixed inset-0 z-50 flex items-center justify-center bg-[#202829]/35 px-4 py-6 backdrop-blur-sm"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      <section
        ...
        className="max-h-[min(780px,calc(100vh-48px))] w-full max-w-[680px] overflow-y-auto bg-[#fbfcfc] p-5 shadow-[0_30px_80px_rgba(45,52,53,0.18)] ring-1 ring-[#adb3b4]/20 md:p-6"
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="关闭弹窗"
            className="inline-flex h-9 w-9 shrink-0 items-center justify-center bg-[#e4e9ea] text-[#5a6061] transition hover:bg-[#dde4e5] hover:text-[#202829]"
          >
            <X size={17} />
          </button>
        </div>
        ...
        {mode === "merchant" ? (
          ...
          <div className="mt-5 bg-[#e8f3ec] p-4 text-sm leading-7 text-[#2f7a4b]">
          ...
          >
            <SubmitFields />
            {error ? (
              <p className="bg-[#fbe9e7] px-3 py-2 text-xs leading-5 text-[#9b3328]">
                {error}
              </p>
            ) : null}
            ...
            <div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end">
              <button
                type="button"
                onClick={onClose}
                className="inline-flex h-10 items-center justify-center bg-[#e4e9ea] px-4 text-sm font-semibold text-[#2d3435] transition hover:bg-[#dde4e5]"
              >
                取消
              </button>
              <button
                type="submit"
                disabled={submitting}
                className="inline-flex h-10 items-center justify-center gap-2 bg-[#2d3435] px-5 text-sm font-semibold text-[#f8f8f8] transition hover:bg-[#1f2526] disabled:cursor-not-allowed disabled:opacity-60"
              >
...
function MerchantContactPanel() {
  ...
  return (
    <div className="mt-5 space-y-4">
      <section className="border border-[#dfe4e5] bg-white p-4">
        <div className="flex items-start gap-3">
          <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center bg-[#e8f3ec] text-[#2f7a4b]">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex h-10 items-center gap-2 bg-[#dde4e5] px-4 text-sm font-semibold text-[#2d3435] transition hover:bg-[#cfd8d9]"
              >
      ...
      <section className="border border-[#dfe4e5] bg-[#f8fafa] p-4">
      ...
      <div className="flex flex-wrap gap-2 text-xs leading-5 text-[#5a6061]">
        <span className="inline-flex items-center gap-1 bg-[#eef3f8] px-3 py-1 text-[#47657a]">
          <MessageCircle className="h-3.5 w-3.5" />
          Telegram: @dimthink
        </span>
        <span className="inline-flex items-center bg-[#eef3f8] px-3 py-1 text-[#47657a]">
          微信: dimthink
        </span>
        <span className="inline-flex items-center bg-[#eef3f8] px-3 py-1 text-[#47657a]">
          邮箱: {merchantEmail}
...
function SubmissionSafetyNote() {
  return (
    <p className="bg-[#fff7e8] px-3 py-2 text-xs leading-5 text-[#7a541b]">
    ...
    <section>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-4">
        {options.map((option) => (
          <label
            key={option}
            className="flex min-h-10 items-center gap-2 border border-[#adb3b4]/20 bg-white px-3 py-2 text-sm font-medium text-[#2d3435]"
          >
            <input
              type="checkbox"
              name={name}
...
const fieldClassName =
  "h-11 w-full border border-[#adb3b4]/30 bg-white px-3 text-sm text-[#202829] outline-none transition placeholder:text-[#6f7778] focus:border-[#2d3435]";
