...
export function TransitViewTabs({
  ...
  return (
    <nav
      className={`inline-flex h-11 items-center gap-1 rounded-full bg-[var(--color-surface)] p-1 ring-1 ring-[var(--color-border-soft)] ${className}`}
      aria-label="中转 API 视图切换"
    >
      <Link
        href={stationHref}
        className={`inline-flex h-9 items-center justify-center gap-1.5 rounded-full px-4 text-sm font-semibold transition-colors ${itemClassName} ${
          active === "stations"
            ? "bg-[var(--color-panel)] text-[var(--color-text-primary)] shadow-[0_8px_24px_rgba(45,52,53,0.05)]"
