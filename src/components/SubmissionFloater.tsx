...
const fieldControlClassName =
  "w-full border border-[var(--color-border-muted)] bg-[var(--color-surface-raised)] px-3 text-sm text-[var(--color-text-body)] outline-none transition placeholder:text-[var(--color-text-placeholder)] focus:border-[var(--color-brand)] focus:ring-2 focus:ring-[#45bf78]/15";
...
export function SubmissionFloater() {
  ...
  return (
    <>
      {open ? (
        <div
          className="priceai-square-ui fixed inset-0 z-50 flex items-center justify-center bg-[var(--color-overlay)] px-4 backdrop-blur-sm"
          onClick={close}
        >
          <div
            className="w-full max-w-md bg-[var(--color-panel)] p-6 shadow-[var(--shadow-floating)] ring-1 ring-[var(--color-border-soft)]"
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <button
                type="button"
                onClick={close}
                className="p-1 text-[var(--color-text-muted)] transition hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)]"
                aria-label="关闭提交渠道窗口"
              >
                <X size={18} />
              </button>
            ...
            {status === "success" ? (
              <div className="mt-5 space-y-3">
                <div className="flex items-start gap-2 border border-[var(--color-border-muted)] bg-[var(--color-success-bg)] px-4 py-3 text-sm text-[var(--color-success-text)]">
            ...
            {status !== "success" ? (
              <form ref={formRef} onSubmit={submit} className="mt-4 space-y-3">
                ...
                {urlsText.trim() ? (
                  <div className="border border-[var(--color-border-soft)] bg-[var(--color-surface)] px-3 py-2 text-xs text-[var(--color-text-muted)]">
                ...
                {status === "error" && message ? (
                  <p className="border border-[var(--color-border-muted)] bg-[var(--color-danger-bg)] px-3 py-2 text-xs text-[var(--color-danger-text)]">
                    {message}
                  </p>
                ) : null}
                ...
                <button
                  ...
                  className="inline-flex h-11 w-full items-center justify-center gap-2 bg-[var(--color-primary)] text-sm font-semibold text-[var(--color-text-on-primary)] transition hover:bg-[var(--color-primary-hover)] disabled:opacity-60"
                >
                  {status === "submitting" ? (
