  ...
  return (
    <>
      ...
      <OfferFilterBar
        ...
        hideFeatureFilters={hideFeatureFilters}
      />
      {loading || !visibleData ? (
        ...
        isDesktop === false ? (
          ...
          <OfferTable offers={offers} onFeedback={setFeedbackOffer} onRequestPurchase={setOutboundOffer} hideFeedback={hideFeedback} />
        )
      ) : (
...
function OfferFilterBar({
  ...
  const visibleFacets = hideFeatureFilters
    ? []
    : Array.from(OFFER_FILTER_TAG_BY_ID.values()).filter((definition) => facetById.has(definition.id));
  ...
  return (
    <section className="mt-6 hidden overflow-hidden rounded-lg bg-white shadow-[0_20px_55px_rgba(45,52,53,0.045)] ring-1 ring-[#adb3b4]/15 md:block">
      <div className="overflow-x-auto">
        <table className={`${hideFeedback ? "min-w-[1136px]" : "min-w-[1200px]"} w-full table-fixed border-collapse text-left text-sm`}>
          <colgroup>
            ...
            {hideFeedback ? null : <col className="w-[64px]" />}
          </colgroup>
          <thead className="bg-[#f2f4f4] text-[0.68rem] font-semibold text-[#5a6061]">
            <tr>
              ...
              {hideFeedback ? null : <TableHead className="text-center">反馈</TableHead>}
            </tr>
          </thead>
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-x-3 gap-y-2">
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
          <OfferActions offer={offer} available={available} onFeedback={onFeedback} onRequestPurchase={onRequestPurchase} hideFeedback={hideFeedback} />
        </div>
      </div>
    </article>
  );
...
export function OfferActions({
  ...
  return (
    <div className="flex flex-nowrap items-center justify-end gap-2">
      ...
      {hideFeedback ? null : <OfferFeedbackButton offer={offer} onFeedback={onFeedback} compact={compact} />}
    </div>
  );
}
